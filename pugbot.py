...
async def check_for_veto(command, context):
    ...
    inputobj = await Bot.wait_for_message(
        timeout=durationOfVeto, author=STARTER[0], check=check
    )
    # wait_for_message returns 'None' if asyncio.TimeoutError thrown
...
async def go_go_gadget_pickup(context):
    ...
    ready_channel = discord.utils.get(
        context.message.server.channels, id=readyupChannelID
    )
    RANDOM_TEAMS = (
        True
    ...
    await send_emb_message_to_channel(
        ...
        + " join the "
        + ready_channel.name
        + " to signify you are present and ready",
        context,
    )
    ...
    while not adminApproves:
        ...
        while not didChoose:
            ...
            inputobj = await Bot.wait_for_message(
                timeout=durationOfCheckin, author=STARTER[0], check=check
            )
            # wait_for_message returns 'None' if asyncio.TimeoutError thrown
            if inputobj != None:
                didChoose = True
...
async def pick_captains(caps, context):
    ...
    emb.add_field(
        name=cmdprefix + "shuffle", value="to randomize the captains", inline=False
    )
    emb.add_field(
        name=cmdprefix + "random", value="to randomize the teams", inline=False
    )
    ...
    if inputobj != None:
        ...
        if inputobj.content.startswith(cmdprefix + "captains"):
            ...
            if bcap == rcap:
                await send_emb_message_to_channel(
                    0xFF0000,
                    STARTER[0].mention
                    + " you cannot pick the same captain for both teams",
                    context,
                )
                return False
            else:
        ...
        if inputobj.content.startswith(cmdprefix + "manual"):
            ...
            while len(BLUE_TEAM) != sizeOfTeams:
                ...
                await send_emb_message_to_channel_blue(
                    STARTER[0].mention
                    + " pick the players for the blue team using "
                    + cmdprefix
                    + "blue <@playername>, <@playername>, ... , <@playername> in your reply.\n\nAvailable players are:\n\n"
                    + plyrStr
                    + "\n\n**Current players** are: "
                ...
                inputobj = await Bot.wait_for_message(
                    timeout=60, author=STARTER[0], check=check
                )
                if inputobj != None:
                    for p in inputobj.mentions:
                        if p not in PLAYERS:
                            await send_emb_message_to_channel(
                                0xFF0000,
                                p.mention
                                + " is not added to the pickup."
                                + STARTER[0].mention
                                + " you will need to pick someone else",
                                context,
                            )
                        else:
                            BLUE_TEAM.append(p)
                    if len(BLUE_TEAM) < sizeOfTeams:
                        ...
                        while len(BLUE_TEAM) != sizeOfTeams:
                            ...
                            await send_emb_message_to_channel(
                                0xFF0000,
                                STARTER[0].mention
                                + " this team has too many players. Pick player(s) to take off using "
                                + cmdprefix
                                + "takeoff <@playername>, <@playername>, ... , <@playername>\n\nCurrent players are:\n\n"
                                + "\n".join([p.mention for p in BLUE_TEAM]),
                                context,
                            )
                            ...
                            while not didRemove:
                                ...
                                if inputobj != None:
                                    for p in inputobj.mentions:
                                        ...
                                        if xtra not in BLUE_TEAM:
                                            await send_emb_message_to_channel(
                                                0xFF0000,
                                                STARTER[0].mention
                                                + " that player is not on the team you will need to pick someone else",
                                                context,
                                            )
                                        else:
                                            # remove this player
                                    ...
                                    await send_emb_message_to_channel(
                                        0xFF0000,
                                        STARTER[0].mention
                                        + " pick the extra player to take off using "
                                        + cmdprefix
                                        + "takeoff @playername in your reply. Current players are:\n\n"
                                        + "\n".join([p.mention for p in BLUE_TEAM]),
                                        context,
                                    )
                    ...
                    await send_emb_message_to_channel_blue(
                        ...
                        + " please "
                        + cmdprefix
                        + "transfer if the game starter is missing",
                        context,
                    )
            # blueTeam is full
...
async def pick_map(context):
    ...
    if VOTE_FOR_MAPS:
        ...
        await send_emb_message_to_channel(
            0x00FF00,
            "Map voting has started\n\n"
            + poolRole.mention
            + " you have "
            + str(durationOfMapVote)
            + " seconds to vote for a map\n\nreply with a number between 1 and "
            + str(sizeOfMapPool)
            + " to cast your vote",
            context,
        )
        ...
        while td.total_seconds() < durationOfMapVote and len(PLAYERS) == sizeOfGame:
            ...
            if (counter % 2) == 1 and (td.total_seconds() < durationOfMapVote):
                await count_votes_message_channel(
                    td, keys, context, votelist, votetotals
                )
            counter += 1
        ...
        if len(votetotals) > 0:
            ...
            for pos, vote in enumerate(votetotals):
                if topvote != vote:
                    continue  # keep looping if they are different
...
async def pickup_is_running(context):
    ...
    if PICKUP_RUNNING:
        ...
        await send_emb_message_to_channel(
            0xFF0000,
            context.message.author.mention
            + " you cannot use this command, there is no pickup running right now. Use "
            + adminRoleMention
            + " to request an admin start one for you",
            context,
        )
        return False
...
async def send_emb_message_to_channel_blue(embstr, context):
    ...
    emb.set_author(
        name="Blue Team",
        icon_url="http://www.lexicondesigns.com/images/other/ff_logo_blue.png",
    )
    await Bot.send_message(context.message.channel, embed=emb)
...
async def send_emb_message_to_channel_red(embstr, context):
    ...
    emb.set_author(
        name="Red Team",
        icon_url="http://www.lexicondesigns.com/images/other/ff_logo_red.png",
    )
    await Bot.send_message(context.message.channel, embed=emb)
...
async def send_information(context):
    ...
    for p in RED_TEAM:
        try:
            ...
            pass
        redTeamMention.append(
            p.mention
        )  # so we can mention all the members of the red team
    for p in BLUE_TEAM:
        try:
            await Bot.send_message(p, embed=emb)
        except Exception:
            pass
        blueTeamMention.append(
            p.mention
        )  # so we can mention all the members of the blue team
    ...
    await Bot.send_message(context.message.channel, embed=emb)
    await send_emb_message_to_channel_blue(
        "\n".join(map(str, blueTeamMention)), context
    )  # Blue Team information
    await send_emb_message_to_channel_red(
        "\n".join(map(str, redTeamMention)), context
    )  # Red Team information
...
async def user_has_access(author):
    if adminRoleID in [r.id for r in author.roles]:
        return True
    return False
...
async def verify_chosen_map_is_good(context):
    ...
    await send_emb_message_to_channel(
        0xFFA500,
        STARTER[0].mention
        + " the map is currently set to: "
        + CHOSEN_MAP[0]
        + "\nReply with "
        + cmdprefix
        + "accept - to **accept** the map or "
        + cmdprefix
        + "repick - to discard the map and all nominations and **repick**",
        context,
    )
    while not didChoose:
        # check for advanced filtering
        def check(msg):
            if msg.content.startswith(cmdprefix + "accept") or msg.content.startswith(
                cmdprefix + "repick"
            ):
                return True
            return False
...
@Bot.command(
    name="add",
    description="Add yourself to the list of players for the current pickup",
    brief="Add to the pickup",
    aliases=["add_me", "addme", "join"],
    pass_context=True,
)
async def _add(context):
    ...
    if context.message.author in PLAYERS:
        await send_emb_message_to_channel(
            0xFF0000,
            context.message.author.mention + " you have already added to this pickup",
            context,
        )
        return
...
@Bot.command(
    name="add_alias",
    description="Adds a new alias to an existing map (row) to the maps collection (table) in the MongoDB",
    brief="Add a new map alias",
    aliases=["update_map", "updatemap"],
    pass_context=True,
)
    ...
    if not await user_has_access(member):
        ...
        if len(message) > 2:
            ...
            if updated:
                await send_emb_message_to_user(
                    0x00FF00,
                    "Map has been updated in the database\n\n"
                    + str(updated["name"])
                    + "\n\nAliases: "
                    + str(updated["aliases"]),
                    context,
                ...
                await send_emb_message_to_user(
                    0xFF0000,
                    "That map does not exist in the database. Did you mean to "
                    + cmdprefix
                    + "addmap?",
                    context,
                )
            ...
            await send_emb_message_to_user(
                ...
                + "addalias ("
                + cmdprefix
                + "updatemap) mapname_or_aliais **newalias1** **newalias2** ... **newalias#**\n\nPlease try again",
                context,
            )
    else:
        await send_emb_message_to_channel(
...
@Bot.command(
    name="addmap",
    description="Adds a new map (row) to the maps collection (table) in the MongoDB",
    brief="Add a new map",
    aliases=["add_map", "new_map", "newmap"],
    pass_context=True,
)
    ...
    if await user_has_access(member):
        ...
        if len(message) > 1:
            ...
            if cursor.count() == 0:
                ...
                await send_emb_message_to_user(
                    0x00FF00,
                    "New map has been added to the database\n\n" + str(last),
                    context,
                )
            else:
                await send_emb_message_to_user(
                    0xFF0000,
                    "That map already exists in the database. Did you mean to "
                    + cmdprefix
                    + "updatemap?",
                    context,
                )
            ...
            await send_emb_message_to_user(
                ...
                + "\n\nThat is not how you use this command, use:\n\n"
                + cmdprefix
                + "addmap <name> <alias1> <alias2> ... <alias##>\n\nPlease try again",
                context,
            )
    else:
        await send_emb_message_to_channel(
...
@Bot.command(
    name="addserver",
    description="Adds a new server (row) to the servers collection (table) in the MongoDB",
    brief="Add a new server",
    aliases=["add_server", "new_server", "newserver"],
    pass_context=True,
)
    ...
    if await user_has_access(member):
        ...
        if len(message) > 4:
            ...
            if re.match(serverIDRegEx, message[4]):
                ...
                await send_emb_message_to_user(
                    0x00FF00,
                    "New server has been added to the database\n\n" + str(last),
                    context,
                )
            else:
                await send_emb_message_to_user(
...
@Bot.command(
    name="admin",
    description="Displays the admin who is in control of the current pickup",
    brief="Displays the admin of the current pickup",
    aliases=["game_admin", "gameadmin", "game_starter", "gamestarter", "starter"],
    pass_context=True,
)
async def _admin(context):
    ...
    if not await pickup_is_running(context):
        ...
        await send_emb_message_to_channel(
            0x00FF00, "Game Admin is: " + STARTER[0].mention, context
        )
        ...
        if len(message) > 4:
            try:
                ...
                if re.match("^[1-9]+[0-9]*$", message[2]):
                    ...
                    # notify the user
                    emb = discord.Embed(
                        description="You have been banned by an Admin for a period of "
                        + str(message[2])
                        + " "
                        + str(message[3])
                        + "\nReason: "
...
@Bot.command(
    name="delmap",
    description="Removes an existing map (row) from the maps collection (table) in the MongoDB",
    brief="Delete an existing server",
    aliases=["del_map", "delete_map", "deletemap"],
    pass_context=True,
)
async def _delmap(context):
    ...
    if await user_has_access(member):
        ...
        if len(message) > 1:
            ...
            await send_emb_message_to_user(
                0x00FF00,
                "Map has been removed from the database\n\n"
                + str(removed)
                + "\n\nNOTE: This action has been logged",
                context,
            )
...
@Bot.command(
    ...
    description="Removes an existing map (row) from the maps collection (table) in the MongoDB",
    brief="Delete an existing server",
    aliases=["del_server", "delete_server", "deleteserver"],
    pass_context=True,
)
    ...
    if await user_has_access(member):
        ...
        if len(message) > 4:
            ...
            if re.match(serverIDRegEx, message[4]):
                ...
                await send_emb_message_to_user(
                    0x00FF00,
                    "Server has been removed from the database\n\n"
                    + str(removed)
                    + "\n\nNOTE: This action has been logged",
                    context,
                )
...
@Bot.command(
    name="end",
    description="Admin only command that ends the current pickup",
    brief="End the current pickup",
    aliases=["edn", "kill", "ned", "stop"],
    pass_context=True,
)
async def _end(context):
    ...
    if await user_has_access(context.message.author):
        ...
        if STARTER[0] == context.message.author or not await check_for_veto(
            cmdprefix + "end", context
        ):
            CHOSEN_MAP = []
            MAP_PICKS.clear()
            for p in PLAYERS:
            ...
            PICKUP_RUNNING = False
            await send_emb_message_to_channel(
                0x00FF00, "The pickup has been ended by an admin", context
            )
            await Bot.change_presence(game=discord.Game(name=" "))
        else:
            await send_emb_message_to_channel(
                0xFF0000, STARTER[0].mention + " has vetoed the command", context
            )
    else:
        await send_emb_message_to_channel(
            0xFF0000,
...
@Bot.command(
    name="hawking",
    description="Displays a random quote from The Great Dr. Hawking",
    brief="Quote from Dr. Stephen Hawking",
    aliases=["hawking_quote", "hawkingquote", "quote"],
    pass_context=True,
)
async def _hawking(context):
    ...
    emb.set_author(
        name="Dr. Stephen William Hawking, 1942-2018", icon_url=Bot.user.avatar_url
    )
...
@Bot.command(
    name="journals",
    description="Displays a link to 55 papers in Physical Review D and Physical Review Letters, gathered together and made public by the American Physical Society",
    brief="Link to Stephen Hawking journals",
    aliases=[
        "aps",
...
async def _journals(context):
    emb = discord.Embed(
        description="""To mark the passing of Stephen Hawking, the American Physical Society have gathered together and made free to read his 55 papers in the peer-reviewed, scientific journals Physical Review D and Physical Review Letters.""",
        colour=0x5E7750,
    )
    emb.set_author(
        name="Dr. Stephen William Hawking, 1942-2018", icon_url=Bot.user.avatar_url
    ...
    emb.add_field(
        name="Link:",
        value="https://journals.aps.org/collections/stephen-hawking",
        inline=False,
    )
    await Bot.send_message(context.message.channel, embed=emb)
...
@Bot.command(
    name="last",
    description="Displays information about the last pickup that was played",
    brief="Show the last pickup info",
    aliases=["last_game", "lastgame", "last_pug", "lastpug"],
    pass_context=True,
)
async def _last(context):
    ...
    emb = discord.Embed(
        title="Last Pickup was " + str(td) + " ago on " + LAST_MAP, colour=0x00FF00
    )
    emb.set_author(name=Bot.user.name, icon_url=Bot.user.avatar_url)
    await Bot.send_message(context.message.channel, embed=emb)
...
@Bot.command(
    name="map",
    description="Show the chosen map for the current pickup",
    brief="Show the selected map",
    aliases=["what_map_won", "whatmapwon"],
    pass_context=True,
)
async def _map(context):
    ...
    if len(CHOSEN_MAP) > 0:
        await send_emb_message_to_channel(
            0x00FF00, "The map for this pickup is " + CHOSEN_MAP[0], context
        )
    else:
        await send_emb_message_to_channel(
            0xFF0000,
            context.message.author.mention
            + " there has not been a map pick for this pickup yet",
            context,
        )
...
@Bot.command(
    name="maps",
    description="List all of the maps that have been nominated for the current pickup",
    brief="Show the nominated maps",
    aliases=["nominated", "nominate_maps", "nominatedmaps"],
    pass_context=True,
)
...
@Bot.command(
    name="maplist",
    description="Provides you with a list of all the maps that are available for nomination via direct message",
    brief="Show the list of maps available",
    aliases=["list_maps", "listmaps", "map_list"],
    pass_context=True,
)
...
@Bot.command(
    name="nominate",
    description="Nominate the map you specified, provided it is valid",
    brief="Nominate the specified map",
    aliases=["elect", "iwanttoplay", "nom"],
    pass_context=True,
)
async def _nominate(context):
    ...
    if len(CHOSEN_MAP) > 0:
        await send_emb_message_to_channel(
            0xFF0000,
            context.message.author.mention
            + " you cannot nominate maps once the map has already been chosen\nCurrent Map: "
            + CHOSEN_MAP[0],
            context,
        )
    else:
        ...
        if context.message.author in PLAYERS:
            ...
            if len(message) > 1:
                ...
                if atom != "INVALID":
                    ...
                    for a, mp in MAP_PICKS.items():
                        if atom == str(mp):
                            await send_emb_message_to_channel(
                                0xFF0000,
                                context.message.author.mention
                                + " that map has already been nominated. Please make another selection",
                                context,
                            )
                    ...
                    if (
                        len(MAP_PICKS) < sizeOfMapPool
                        or context.message.author in MAP_PICKS
                    ):
                        # users may only nominate one map
                        MAP_PICKS.update({context.message.author: atom})
                        await send_emb_message_to_channel(
                            0x00FF00,
                            context.message.author.mention + " has nominated " + atom,
                            context,
                        )
                    else:
                        # need to build the list of maps
                        ...
                        for k in MAP_PICKS:
                            mapStr = (
                                mapStr + str(MAP_PICKS[k]) + " (" + k.mention + ")\n"
                            )
                        emb = discord.Embed(
                            description=context.message.author.mention
...
@Bot.command(
    name="pickup",
    description="Start a new pickup, if one is not already currently running",
    brief="Start a new pickup",
    aliases=[
        "new_game",
        "newgame",
        "new",
...
async def _pickup(context):
    ...
    if await user_has_access(context.message.author):
        ...
        if PICKUP_RUNNING:
            await send_emb_message_to_channel(
                0xFF0000,
                "There is already a pickup running. "
                + cmdprefix
                + "teams to see the game details",
                context,
            )
        else:
            PICKUP_RUNNING = True
            ...
            await send_emb_message_to_channel(
                0x00FF00,
                "A pickup has been started. " + cmdprefix + "add to join up.",
                context,
            )
            await Bot.change_presence(
                game=discord.Game(
...
@Bot.command(
    name="players",
    description="Change the number of players per team (Admin only)",
    brief="Change the number of players in pickup",
    aliases=[
        "players_per_team",
        "size_of_teams",
        "sizeofteams",
    ...
    if await user_has_access(context.message.author):
        ...
        if STARTER[0] == context.message.author:
                ...
                try:
                    ...
                    if sz == 0:
                        ...
                        await send_emb_message_to_channel(
                            0xFF0000,
                            context.message.author.mention
                            + " you cannot change to zero players, please use "
                            + cmdprefix
                            + "end instead",
                            context,
                        )
                        ...
                        if sz < len(PLAYERS):
                            ...
                            await send_emb_message_to_channel(
                                0xFF0000,
                                context.message.author.mention
                                + " the player pool is too big to change to that value",
                                context,
                            )
                        else:
                            ...
                            await send_emb_message_to_channel(
                                0x00FF00,
                                context.message.author.mention
                                + " the size of the game has been changed to "
                                + str(sz),
                                context,
                            )
                            await Bot.change_presence(
                        ...
                        await send_emb_message_to_channel(
                            0xFF0000,
                            context.message.author.mention
                            + " the size of the teams must be even",
                            context,
                        )
                except (ValueError):
                    await send_emb_message_to_channel(
            ...
            await send_emb_message_to_channel(
                0xFF0000,
                context.message.author.mention
                + " sorry, this pickup does not belong to you, it belongs to "
                + STARTER[0].mention,
                context,
            )
    else:
...
@Bot.command(
    name="pug",
    description="Allow user access to the pickup channel ",
    brief="Give you access to the channel",
    aliases=["letmein"],
    pass_context=True,
)
async def _pug(context):
    ...
    try:
        ...
        await send_emb_message_to_channel(
            0x00FF00, "Successfully added role {0}".format(accessRole.name), context
        )
    except (discord.Forbidden, discord.HTTPException):
        pass
...
@Bot.command(
    ...
    brief="Answers from the pugbot",
    aliases=["8ball", "8-ball", "eight_ball", "eightball"],
    pass_context=True,
)
...
@Bot.command(
    name="remove",
    description="Remove yourself or the specified player (Admin only) and any map nomination they may have from the pickup",
    brief="Removes the user from the pickup",
    aliases=["removeme", "remove_me", "removeplayer", "remove_player"],
    pass_context=True,
)
    ...
    try:
        ...
        if await user_has_access(context.message.author):
            if idleUser in PLAYERS:
                ...
                await send_emb_message_to_channel(
                    ...
                    + " you have been removed from the pickup by "
                    + context.message.author.mention
                    + " (admin)",
                    context,
                )
                ...
                await send_emb_message_to_channel(
                    0x00FF00,
                    context.message.author.mention
                    + " that user is not added to the pickup",
                    context,
                )
        else:
            await send_emb_message_to_channel(
        ...
        if idleUser in PLAYERS:
            ...
            await send_emb_message_to_channel(
                0x00FF00,
                context.message.author.mention
                + " you have been removed from the pickup",
                context,
            )
            await Bot.change_presence(
                game=discord.Game(
            ...
            await send_emb_message_to_channel(
                0x00FF00,
                context.message.author.mention + " no worries, you never even added",
                context,
            )
...
@Bot.command(
    name="removenom",
    description="Allows an admin to remove a map nominate from the list",
    brief="Removes the specified map nomination from the pickup",
    aliases=[
        "removenomination",
        "remove_nom",
        "remove_nomination",
    ...
    if await user_has_access(context.message.author):
        ...
        if len(message) > 1:
            ...
            if atom != "INVALID":
                ...
                for author, mp in MAP_PICKS.items():
                    if atom == str(mp):
                        ...
                        await send_emb_message_to_channel(
                            0x00FF00,
                            atom
                            + " has been removed from the nominations by "
                            + context.message.author.mention
                            + " (admin)",
                            context,
                        )
                ...
                # no match found
                await send_emb_message_to_channel(
                    0xFF0000, atom + " is not a nominated map currently", context
                )
            else:
                await send_emb_message_to_channel(
                    0xFF0000,
                    context.message.author.mention
                    + " that mapname is not a valid map. Please make another selection",
                    context,
                )
        else:
            await send_emb_message_to_channel(
...
@Bot.command(
    name="setmode",
    description="Allows an admin to change the way the map is picked. Options are random or vote",
    brief="Change the way the map is picked",
    aliases=["changemode", "change_mode", "set_mode"],
    pass_context=True,
)
        ...
        if STARTER[0] == context.message.author:
            ...
            try:
                ...
                if m.startswith("random"):
                    ...
                    await send_emb_message_to_channel(
                        0x00FF00,
                        "Map Selection has successfully been changed to randomly select from the list of nominations",
                        context,
                    )
                    ...
                    await send_emb_message_to_channel(
                        0x00FF00,
                        "Map Selection has successfully been changed to call a player vote",
                        context,
                    )
                else:
                    await send_emb_message_to_channel(
                        0xFF0000,
                        context.message.author.mention
                        + " that is not a valid mode you must type "
                        + cmdprefix
                        + "setmode random or "
                        + cmdprefix
                        + "setmode vote",
                ...
                await send_emb_message_to_channel(
                    0xFF0000,
                    context.message.author.mention
                    + " to change the map selection mode you must type "
                    + cmdprefix
                    + "setmode random or "
                    + cmdprefix
                    + "setmode vote",
            ...
            if len(message) > 2:
                ...
                if cursor is not None and cursor.count() > 0:
                    ...
                    await send_emb_message_to_channel(
                        0xFF0000,
                        context.message.author.mention
                        + " I am sorry, I did not find a server matching "
                        + op
                        + " = "
                        + data
                        + "\n\nPlease try again",
...
@Bot.command(
    name="sendinfo",
    description="Sends the server IP and password via direct message",
    brief="Send server info via DM",
    aliases=["send_info"],
    pass_context=True,
)
...
@Bot.command(
    name="teams",
    description="Displays all of the members in the current pickup",
    brief="Display current pickup info",
    aliases=["game_info", "gameinfo"],
    pass_context=True,
)
async def _teams(context):
    ...
    if len(PLAYERS) < 1:
        await send_emb_message_to_channel(
            0x00FF00,
            "The pickup is empty right now. " + cmdprefix + "add to join",
            context,
        )
    elif len(PLAYERS) > 0:
        plyrStr = "\n".join([p.mention for p in PLAYERS])
...
@Bot.command(
    name="transfer",
    description="Give your pickup to another admin (Game Starter) or take possesion of another admins pickup (All Other Admins)",
    brief="Transfer the current pickup to/from another admin",
    aliases=["give", "take"],
    pass_context=True,
)
        ...
        if STARTER[0] == context.message.author:
                ...
                if newCap != STARTER[0]:
                    ...
                    if await user_has_access(newCap):
                        ...
                        await send_emb_message_to_channel(
                            0x00FF00,
                            context.message.author.mention
                            + " your pickup has successfully been transfered to "
                            + newCap.mention,
                            context,
                        )
                    else:
                        await send_emb_message_to_channel(
                            0xFF0000,
                            context.message.author.mention
                            + " you may only transfer your pickup to another admin",
                            context,
                        )
                else:
                    await send_emb_message_to_channel(
                        0xFF0000,
                        context.message.author.mention + " you already own this pickup",
                        context,
                    )
            except (IndexError):
                await send_emb_message_to_channel(
                    ...
                    + " you must type "
                    + cmdprefix
                    + "transfer @nameOfAdmin to transfer your pickup.",
                    context,
                )
            ...
            try:
                ...
                await send_emb_message_to_channel(
                    0xFF0000,
                    context.message.author.mention
                    + " you may only transfer your own pickup to another player",
                    context,
                )
                ...
                if not await check_for_veto(cmdprefix + "transfer", context):
                    await send_emb_message_to_channel(
                        0x00FF00,
                        context.message.author.mention
                        + " you have successfully taken possesion of "
                        + STARTER[0].mention
                        + " pickup",
                        context,
                    )
                    ...
                    await send_emb_message_to_channel(
                        0xFF0000,
                        STARTER[0].mention + " has vetoed the transfer",
                        context,
                    )
    else:
        await send_emb_message_to_channel(
...
async def _unban(context):
    ...
    if await user_has_access(context.message.author):
        try:
            ...
            await send_emb_message_to_channel(
                0xFF0000,
                "That is not a valid user, you must @mention the player\n\n"
                + cmdprefix
...
@Bot.command(
    name="unsubscribe",
    description="Allows users to leave the notification group, which removes them from the channel",
    brief="Leave the pickup channel",
    aliases=["unpug", "unsub"],
    pass_context=True,
)
async def _unsubscribe(context):
    ...
    if accessRole is not None:
        try:
            ...
            await send_emb_message_to_user(
                0x00FF00,
                "Successfully removed role {0}".format(accessRole.name),
                context,
            )
        except (discord.Forbidden, discord.HTTPException):
            pass
